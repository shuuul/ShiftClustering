...
class GridShift:
    ...
    def draw_box(self, X, x1, x2, y1, y2, cluster_vals=[0], mask_val=1):
        ...
        result, __ = self.fit_predict(X_box, self.max_iters)
    ...
    def track(self, X, center, mask, hist, length, width, mask_val=1, adjust_threshold=0.75):
        ...
        while not self.max_iters or iteration < self.max_iters:
            iteration += 1
            ...
            if obj[0].size == 0:
                ...
                new_center, hist, length, width = self.update_hist(X, mask, weigth, hist, length, width,
                                                                   mask_val=mask_val)
    ...
    def fit_predict(self, X, return_centers=False):
        ...
        centers = X_shifted[0:np.ndarray.item(k), :];
        if return_centers:
